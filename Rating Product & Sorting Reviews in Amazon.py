...
df = pd.read_csv("/Users/melisacevik/PycharmProjects/Amazon-Projects/amazon_review.csv")
df.head()

product_avg = df.groupby("asin").agg({"overall":"mean"})
...
def total_vote_and_up_diff(total_vote, up):
    return total_vote - up

df["helpful_no"] = total_vote_and_up_diff(df["total_vote"], df["helpful_yes"])
df.head(40)
